...
app.relocator = function (row_col,pos) {
  ...
  if (pos[0]==='!') {
    ...
    if (from) {
      console.log( 'locality array: ' + JSON.stringify(active_stanza.locality) )

      //
      if (!app.mapper.entrances[from]) {
        console.log('from')
        console.log(from)
        console.log( 'query selector: #' + this_pos + target)
...
active_map.shift_stanza_dom = function(stanza_data) {
  ...
  Object.keys(stanza_data.theme.glyph_char).forEach( (id) => {
    //console.log('icon for element ' + id )
    document.querySelector('#' + id).innerHTML =
      icons[stanza_data.theme.glyph_char[id].char]
    //console.log(stanza_data.theme.glyph_char)
    //console.log(icons)
  })
}
