...
app.relocator = function (row_col,pos) {
  ...
  if (el) {
    ...
    if (from) {
      console.log( 'locality array: ' + active_stanza.locality )
      console.log( 'query selector: #' + app.mapper.entrances[pos.replace('!','')] )
...
app.shift_stanza = function (dir) {
  ...
  if (active_stanza.locality[dir]) {
    ...
    story_index = dir==='asc' ? active_stanza.story+1 :
                  dir==='desc' ? active_stanza.story-1 :
                  story_index
  ...
  return this.stanza_constructor(
    story_index,
    id_str,
    active_map.stories[story_index+1][id_str]
  )
}
...
active_map.shift_stanza_dom = function(stanza_data) {
  ...
  Object.keys( icons ).forEach( (key) => {
    icons[key].className = stanza_data.locality[key] ?
      icons[key].className.replace('closed','open') :
      icons[key].className.replace('open', 'closed')
  })
}
