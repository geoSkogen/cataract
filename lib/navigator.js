...
app.relocator = function (row_col,pos) {
  ...
  // if there's a dom element for the tile you requested directions to
  if (el) {
    ...
    // simple navigation between stanza tiles
    if (pos[0]!='!') {
  ...
  // executables - including navigation between stanzas and between stories
  if (pos[0]==='!') {
    ...
    if (active_stanza.locality[pos.replace('!','')]) {
      ...
      // clear the glyphs for reset . . .
      glyphs.forEach( (glyph) => {
        glyph.innerHTML = ''
      })
    } else {
      // the requested excutable is not for a locality
      this.saga[ pos.replace('!','') ] = this.encounters[ pos.replace('!','') ]()
    }

    if (from) {
      // reassign the active glyph  . . .
      if (!app.mapper.entrances[from]) {
      // . . . not to occupy the same tile as the executable
        if (!former_id) {
